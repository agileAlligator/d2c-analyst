"""Margin Watch agent — scans order-level contribution margin and proposes ₹-saving actions."""
...
class MarginWatchAgent(BaseAgent):
                ...
                self.emit_proposal(
                    Proposal(
                        ...
                        would_do_api_call={
                            ...
                            "note": (
                                f"Raise price by ₹{abs(margin):.2f} to reach breakeven"
                                " — exact variant must be determined from order line items"
                            ),
                            "NOT_SENT": True,
                        },
                    )
    ...
    def _propose_courier_switch(self):
        ...
        result = query_metric(self.db, self.merchant_id, "rto_rate", group_by="courier", time_range="30d")
        if not result.rows:
    ...
    def _propose_adset_pause(self):
        ...
        spend_result = query_metric(self.db, self.merchant_id, "ad_spend", group_by="campaign", time_range="14d")
        if not spend_result.rows:
            self.log("No ad spend data available.")
            return

        revenue_result = query_metric(self.db, self.merchant_id, "revenue", time_range="14d")

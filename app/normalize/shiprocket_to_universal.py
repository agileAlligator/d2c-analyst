...
def _upsert_shipment(db: Session, merchant_id: str, raw: RawShiprocketShipment):
    ...
    prov_record(
        db, merchant_id, "entities", str(entity_id), "raw_shiprocket_shipments", raw.source_record_id, TRANSFORM_ID
    )
    ...
    if occurred_at and freight > 0:
        ...
        prov_record(
            db, merchant_id, "events", str(event_id), "raw_shiprocket_shipments", raw.source_record_id, TRANSFORM_ID
        )

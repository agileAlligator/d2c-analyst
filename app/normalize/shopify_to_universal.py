...
def _upsert_order(db: Session, merchant_id: str, raw: RawShopifyOrder):
    ...
    prov_record(db, merchant_id, "entities", str(entity_id), "raw_shopify_orders", raw.source_record_id, TRANSFORM_ID)
    ...
    if occurred_at and amount > 0:
        event_id = _upsert_event(
            ...
            None,
            {
                ...
                "shipping": str((p.get("total_shipping_price_set") or {}).get("shop_money", {}).get("amount", "0")),
                "line_items": [
                    {
                        "sku": li.get("sku"),
                        "title": li.get("title"),
        ...
        prov_record(db, merchant_id, "events", str(event_id), "raw_shopify_orders", raw.source_record_id, TRANSFORM_ID)
...
def _upsert_refund(db: Session, merchant_id: str, raw: RawShopifyRefund):
    ...
    total_refund = sum(
        Decimal(str(t.get("amount", "0"))) for t in p.get("transactions", []) if t.get("kind") == "refund"
    )
    occurred_at = _parse_dt(p.get("created_at"))
    if occurred_at and total_refund > 0:
        ...
        prov_record(db, merchant_id, "events", str(event_id), "raw_shopify_refunds", raw.source_record_id, TRANSFORM_ID)


def _upsert_entity(
    db: Session, merchant_id: str, entity_type: str, natural_key: str, source: str, attributes: dict
) -> uuid.UUID:
...
def _get_or_create_entity(
    ...
    row = db.query(Entity.entity_id).filter_by(merchant_id=merchant_id, natural_key=natural_key).one()

...
class TestAccuracy:
    ...
    def test_highest_rto_courier_is_shadowfax(self):
        """Shadowfax has ~21.7% RTO rate — highest of all couriers."""
    ...
    def test_campaign_spend_names_a_campaign(self):
        ...
        assert any(c in r["answer"] for c in campaigns), f"Expected a campaign name. Answer: {r['answer']}"
...
class TestMerchantIsolation:
    ...
    def test_revenue_differs_between_merchants(self):
        ...
        if not _has_numbers(r_demo["answer"]) or not _has_numbers(r_demo2["answer"]):
            pytest.skip(
                "One or both answers contain no numbers (likely timeout boilerplate) — skipping isolation check"
            )

        assert r_demo["answer"] != r_demo2["answer"], "demo and demo2 returned identical revenue — RLS isolation broken"
    ...
    def test_demo2_has_lower_revenue(self):
        ...
        if demo_rev == 0 or demo2_rev == 0:
            pytest.skip(
                "One or both answers contain no numbers (likely timeout boilerplate) — skipping revenue comparison"
            )

        assert demo_rev > demo2_rev, f"demo revenue ({demo_rev}) should exceed demo2 ({demo2_rev})"
...
class TestFullScorecardAndWrite:
    ...
    def test_full_scoreboard(self):
        ...
        failing = [r["question"] for r in results if not r["valid"] and "ADVERSARIAL" not in r.get("description", "")]
        assert non_adv_pct >= 100, (
            f"Non-adversarial citation coverage {non_adv_pct:.0f}% — must be 100%. "
            f"Failing questions: {failing}"
        )
        assert adv_pct >= 66, f"Adversarial citation coverage {adv_pct:.0f}% below 66% floor"
        assert accuracy_pct >= 60, f"Accuracy {accuracy_pct:.0f}% below 60% target"
